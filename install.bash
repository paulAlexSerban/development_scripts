...
# check if the entry.bash is already sourced in the .zshrc file and if not, add it on the last line
# grep -q 'source ~/development_scripts/entry.bash' ~/.zshrc || echo 'source ~/development_scripts/entry.bash' >> ~/.zshrc

if grep -q 'source ~/development_scripts/entry.bash' ~/.zshrc; then
  echo "entry.bash is already sourced in the .zshrc file"
else
  echo '' >>~/.zshrc
  echo "source ~/development_scripts/entry.bash" >>~/.zshrc
fi
